...
int LinearSolver::calculateHeadingAngle(int teta_goal)
{
    _distanceSensorData = _dataProvider->getSample();
    calculateForces(teta_goal);
    int angle = std::min_element(std::begin(_forces.totalFieldData), std::end(_forces.totalFieldData),
...
std::vector<Solver::DistanceSensorData> LinearSolver::calculateAttractiveField(int teta_goal)
{
    std::vector<DistanceSensorData> attrFieldData;
    for (size_t i = 0; i < _distanceSensorData.size(); ++i) // distance sensor data is used, cause it holds angles.
    {
        double value = SolverParams::_gamma * fabs(teta_goal - _distanceSensorData[i].angle);
