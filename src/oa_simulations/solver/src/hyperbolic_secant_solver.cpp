...
double calculate_val(double theta, double Teta_k, double sigma, double A) 
{
    ...
    double underExp =  0.5 * M_PI * (Teta_k - theta) / sigma;
    return A * sech(underExp);
}
}
...
int HyperbolicSecantSolver::calculateHeadingAngle(int teta_goal)
{
...
std::vector<DistanceSensorData> HyperbolicSecantSolver::calculateRepulsiveField()
{
...
std::vector<DistanceSensorData> HyperbolicSecantSolver::calculateAttractiveField(int teta_goal)
{
